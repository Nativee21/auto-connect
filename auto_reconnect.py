...
def focus_rust_window():
    """Bring the Rust window to the foreground."""
    try:
        ...
        log(f"❌ focus_rust_window error: {e}")

    # Fallback to alt-tab if activation failed
...
def wait_until_in_game(timeout=90):
    """Wait until the in-game screen is detected."""
    start_time = time.time()
    while time.time() - start_time < timeout:
        if is_in_game():
            return True
...
def post_load_sequence(timeout=60):
    """Handle state right after the loading screen appears."""
    start = time.time()
    while time.time() - start < timeout:
        if is_dead():
            log("☠️ Dead after loading. Respawning...")
            try_click_respawn()
            time.sleep(2)
            continue
        if is_asleep():
            log("😴 Asleep after loading. Waking up...")
            try_click_to_wake()
            time.sleep(2)
            continue
        if is_in_game():
            log("🏁 Joined server. Starting farming...")
            simulate_human_movement()
            return True
        time.sleep(1)
    log("❌ Could not reach in-game state in time.")
    return False
...
def automation_loop():
    ...
    while running:
        ...
        if is_in_game():
            ...
            simulate_human_movement()
            time.sleep(random.randint(20, 40))  # Shorter cooldown between movements
            continue
        ...
        log("🤔 Unknown state. Waiting 10 seconds...")
        time.sleep(10)
...
async def start(ctx: discord.ApplicationContext):
    ...
    if not is_rust_running():
        ...
        launch_rust()
        # wait up to 60 seconds for Rust to launch
        for _ in range(60):
            if is_rust_running():
                break
            time.sleep(1)
